...
exports.verifyRules = exports.doCotWithRenta = exports.doCotCliente = exports.doCotizacion = void 0;
...
const obtenerValoresOtrosGastos = (plazo) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const tablaOtrosGastos = yield valor_otros_gastos_1.Valor_Otros_Gastos.findAll({
            where: { deleted: false },
            attributes: [
                "id",
                "plazo",
        ...
        const valorOtrosGastos = tablaOtrosGastos.find((valor) => plazo <= valor.dataValues.plazo);
        return valorOtrosGastos === null || valorOtrosGastos === void 0 ? void 0 : valorOtrosGastos.dataValues.total;
    }
    catch (error) {
        console.log(error);
    }
});
const obtenerCamposEditables = (campo) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const tablaCamposEditables = yield editable_1.Editable.findAll({
            where: { deleted: false },
            attributes: ["campo", "valor", "tipo"],
        });
        const porcentajePagoIncial = tablaCamposEditables.find((obj) => obj.dataValues.campo === campo);
        return porcentajePagoIncial === null || porcentajePagoIncial === void 0 ? void 0 : porcentajePagoIncial.dataValues.valor;
    }
    catch (error) {
        console.log(error);
    }
});
const obtenerTasas = (plan, tipoActivo, plazo) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const tablaTasas = yield valor_tasas_1.Valor_Tasas.findAll({
            where: { deleted: false },
            attributes: [
                "tipo_activo",
                "plazo",
        ...
        const valorTasas = tablaTasas.find((valor) => valor.dataValues.tipo_activo === tipoActivo &&
            valor.dataValues.plazo >= plazo);
        if (!valorTasas) {
            throw new Error("No se encontró un valor de tasa para el tipo de activo y plazo proporcionados");
        }
        switch (plan) {
            case "tasa_a":
...
const selectedValue = (value, valueModified) => {
    let newValue = 0;
    if (value != valueModified) {
        if (valueModified === 0)
            newValue = value;
        if (valueModified > value)
            newValue = valueModified;
    }
...
const calculateValuesRenta = (renta) => {
...
const getResidual = (plazo) => __awaiter(void 0, void 0, void 0, function* () {
    const valoresResiduales = yield valor_residual_1.Valor_Residual.findOne({
        where: { plazo, deleted: false },
        attributes: ["id", "plazo", "minimo", "maximo"],
    });
    if (!valoresResiduales) {
        const valorResidual = yield valor_residual_1.Valor_Residual.findOne({
            where: {
...
const getResiduales = (plazo) => __awaiter(void 0, void 0, void 0, function* () {
    const valoresResiduales = yield valor_residual_1.Valor_Residual.findOne({
        where: { plazo, deleted: false },
        attributes: ["id", "plazo", "minimo", "maximo"],
    });
    if (!valoresResiduales) {
        ...
        const resp = {
            isValid: true,
            data: valorResidual.dataValues,
        };
        return resp;
    }
    const respIsCorrect = {
        isValid: true,
        data: valoresResiduales.dataValues,
    };
    return respIsCorrect;
});
const isBetween = (principal, inferiror, superior) => {
    return principal >= inferiror && principal <= superior;
};
const doCalculatePayment = (
// plazos: number[],
plazo, precioSinIVA, costoAccesoriosSinIVA, pagoInicialSinIVA, tipoTasa, tipoActivo, porcentajeComision) => __awaiter(void 0, void 0, void 0, function* () {
    const resultados = []; // Aquí se declara como un arreglo de objetos 'renta'
    ...
    for (const plazo of plazos) {
        // // ***********************************************************************************************************
        const otrosGastos = (yield obtenerValoresOtrosGastos(plazo)) || 0;
        // // ***********************************************************************************************************
        const tasa = (yield obtenerTasas(tipoTasa, tipoActivo, plazo)) || 0.032;
        // // ***********************************************************************************************************
        let montoDelArrendamientoSinIVA = precioSinIVA + costoAccesoriosSinIVA + otrosGastos - pagoInicialSinIVA;
        const tasaArmada = parseFloat((tasa / 12 / 100).toFixed(4));
        let valorResidualConvenido = yield getResidual(plazo);
        ...
        const renta = calculatePayment(tasaArmada, plazo, -montoDelArrendamientoSinIVA, residual, 0);
        const { ivadeRentaMensual, rentaMensualTotal } = calculateValuesRenta(renta);
        const resultado = {
            plazo,
            renta,
            ivadeRentaMensual,
...
const doCotizacion = (cot) => __awaiter(void 0, void 0, void 0, function* () {
    // try {
    const { cliente, enAtencionA, tipoCliente, promotor, correo, telefono, tipoActivo, cantidadUnidades, marca, modelo, version, estado, valorFactura, plazo, comisionApertura, totalPagoInicial, plan, tipoSeguro, costoSeguro, rentasDeposito, tipoResidual, fondoReserva, valorResidualConvenido, accesorios, } = cot;
    console.log("*******************************************", "Aquí están todos los datos:", cot, "******************************************************************************************");
    // // ***********************************************************************************************************
    const otrosGastos = (yield obtenerValoresOtrosGastos(plazo)) || 0;
    console.log("El valor de los otros gastos es: ", otrosGastos);
    // // ***********************************************************************************************************
    const tasa = (yield obtenerTasas(plan, tipoActivo, plazo)) || 0.032;
    console.log("La tasa que se tomará en cuenta es: ", tasa);
    // // ***********************************************************************************************************
    const precioSinIVA = valorFactura / 1.16;
    console.log("============================================ACCESORIOS=====================================");
    const costoAccesoriosSinIVA = Object.keys(accesorios).reduce((total, acc, index) => {
    ...
    if (comisionApertura < 100) {
        let comisionPorApertura = montoDelArrendamientoSinIVA * (comisionApertura / 100);
    ...
    const rentaMensual = calculatePayment(tasaArmada, plazo, -montoDelArrendamientoSinIVA, residual, 0);
    ...
    if (rentasDeposito !== 0) {
        const porcentajeDescPorRenta = (yield obtenerCamposEditables("Descuento mensual por rentas")) || 0;
        ...
        console.log("El nuevo valor inicial es:", nuevosParametrosRenta[0].valorInicial);
        ...
        if (comisionApertura < 100) {
            let comisionPorApertura = nuevosParametrosRenta[0].montoArrendamiento * (comisionApertura / 100);
            comisionAp = comisionPorApertura;
        ...
        nuevosParametrosRenta[0].rentaMensual = calculatePayment(tasaArmada, plazo, -nuevosParametrosRenta[0].montoArrendamiento, residual, 0);
        console.log(JSON.stringify({
        ...
        console.log("El subtotal antes del IVA es: ", nuevosParametrosRenta[0].totalRentaMensual);
        if (fondoReserva) {
            nuevosParametrosRenta[0].fondoReserva =
                nuevosParametrosRenta[0].totalRentaMensual * (fondoReserva / 100);
        }
        console.log("**********El fondo de reserva expresado en cantidad es: ", nuevosParametrosRenta[0].fondoReserva);
        nuevosParametrosRenta[0].granTotalRentaMensual =
            nuevosParametrosRenta[0].ivaRenta +
                nuevosParametrosRenta[0].totalRentaMensual +
                nuevosParametrosRenta[0].fondoReserva;
        console.log("El total de la renta mensual con IVA más el sutotal es: ", nuevosParametrosRenta[0].granTotalRentaMensual);
    }
    ...
    console.log("**********El fondo de reserva expresado en cantidad es: ", fondoReservaCantidad);
    const rentaMensualTotal = ivadeRentaMensual + subtotalRentaMensual + fondoReservaCantidad;
    console.log("El total de la renta mensual con IVA más el sutotal es: ", rentaMensualTotal);
    // ----------------------------------- // ------------------------------ // ---------------------------- // -----------------------------------
    ...
    // );
    // ************************************************************************* //
    const resultadosRentas = yield doCalculatePayment(plazo, precioSinIVA, costoAccesoriosSinIVA, pagoInicialSinIVA, plan, tipoActivo, comisionApertura);
    const opc_rentas = [];
    if (resultadosRentas.length > 0) {
        for (let index = 0; index < resultadosRentas.length; index++) {
            const opcionesValores = {
    ...
    const resultados = {
        ...
        montoArrendamientoFinal: selectedValue(montoDelArrendamientoSinIVA, nuevosParametrosRenta[0].montoArrendamiento),
        comisionApertura: comisionAp,
        valorInicialArrendamiento: valorInicialArrendamiento != nuevosParametrosRenta[0].valorInicial
        ...
        rentaMensual: selectedValue(rentaMensual, nuevosParametrosRenta[0].rentaMensual),
        rentasDeposito: rentasDeposito === 0 ? rentasDeposito : nuevaRentaEnDeposito,
        descuentoMxRD: descuento,
        subtotalRentaMensual: nuevosParametrosRenta[0].totalRentaMensual != 0
            ? nuevosParametrosRenta[0].totalRentaMensual
            : subtotalRentaMensual,
        ivadeRentaMensual: nuevosParametrosRenta[0].ivaRenta != 0
            ? nuevosParametrosRenta[0].ivaRenta
            : ivadeRentaMensual,
        fondoReserva: nuevosParametrosRenta[0].fondoReserva != 0
            ? nuevosParametrosRenta[0].fondoReserva
            : fondoReservaCantidad,
        rentaMensualTotal: nuevosParametrosRenta[0].granTotalRentaMensual != 0
            ? nuevosParametrosRenta[0].granTotalRentaMensual
            : rentaMensualTotal,
        opciones: opc_rentas,
    };
...
const doCotCliente = (cot) => __awaiter(void 0, void 0, void 0, function* () {
    const resultados = {
        tipoActivo: "",
        plazo: 0,
        valorFactura: 0,
    ...
    const tasa = tipoActivo === "Bicicletas"
        ? 45
        : (yield obtenerTasas(tipoTasa, tipoActivo, plazo)) || 32;
    console.log("La tasa que se tomará en cuenta es: ", tasa);
    // // ***********************************************************************************************************
    // console.log(
    //   JSON.stringify(
    ...
    //   )
    // );
    let montoDelArrendamientoSinIVA = precioSinIVA + otrosGastos - pagoInicialSinIVA;
    const tasaArmada = parseFloat((tasa / 12 / 100).toFixed(4));
    let valorResidualConvenido = yield getResidual(plazo);
    ...
    if (tipoActivo === "Bicicletas" || tipoActivo === "Motocicleta") {
        const { ivadeRentaMensual, rentaMensualTotal } = calculateValuesRenta(rentaMensual);
    ...
    const resultadosRentas = yield doCalculatePayment(plazo, precioSinIVA, costoAccesoriosSinIVA, pagoInicialSinIVA, tipoTasa, tipoActivo, porcentajeComision);
    const { ivadeRentaMensual, rentaMensualTotal } = calculateValuesRenta(rentaMensual);
    resultados.plazo = plazo;
    resultados.tipoActivo = tipoActivo;
    resultados.valorFactura = valorFactura;
...
exports.doCotCliente = doCotCliente;
// Se modificará la renta
const doCotWithRenta = (cot) => __awaiter(void 0, void 0, void 0, function* () {
    // try {
    const { cliente, enAtencionA, tipoCliente, tipoActivo, cantidadUnidades, valorFactura, plazo, comisionApertura, totalPagoInicial, plan, tipoSeguro, costoSeguro, rentasDeposito, tipoResidual, fondoReserva, valorResidualConvenido, accesorios, isEspecial, } = cot;
    // console.log(isEspecial);
    // // ***********************************************************************************************************
    const otrosGastos = (yield obtenerValoresOtrosGastos(plazo)) || 0;
    // console.log("El valor de los otros gastos es: ", otrosGastos);
    // // ***********************************************************************************************************
    const tasa = (yield obtenerTasas(plan, tipoActivo, plazo)) || 32;
    // console.log("La tasa que se tomará en cuenta es: ", tasa);
    // // ***********************************************************************************************************
    const residuales = yield getResiduales(plazo);
    ...
    if (residuales.isValid) {
        ...
        const valorResidual = residuales.data;
    ...
    const rentaMensualTotal = ivadeRentaMensual + subtotalRentaMensual + fondoReservaCantidad;
    const resultadosRentas = yield doCalculatePaymentv2(plazo, precioSinIVA, costoAccesoriosSinIVA, pagoInicialSinIVA, plan, tipoActivo, comisionApertura, fondoReserva, rentasDeposito, totalPagoInicial, precioSeguro);
    const opc_rentas = [];
    if (resultadosRentas.length > 0) {
        for (let index = 0; index < resultadosRentas.length; index++) {
            const opcionesValores = {
    ...
    const resultados = {
        ...
        opciones: tipoActivo === "Bicicletas" || tipoActivo === "Motocicleta"
            ? []
            : opc_rentas,
    };
    return {
...
exports.doCotWithRenta = doCotWithRenta;
const verifyRules = (valorFactura, totalPagoInicial, tipoSeguro, costoSeguro, rentasDeposito) => {
    const respuesta = {
        error: "",
        isValid: true,
    };
...
const doCalculatePaymentv2 = (
// plazos: number[],
plazo, precioSinIVA, costoAccesoriosSinIVA, pagoInicialSinIVA, tipoTasa, tipoActivo, porcentajeComision, fondoReserva, rentasDeposito, totalPagoInicial, costoSeguro) => __awaiter(void 0, void 0, void 0, function* () {
    const resultados = []; // Aquí se declara como un arreglo de objetos 'renta'
    const plazos = [12, 24, 36, 48];
    switch (plazo) {
        case 12:
